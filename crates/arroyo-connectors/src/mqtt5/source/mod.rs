...
impl MqttSourceFunc {
    ...
    async fn run_int(
        ...
        // Build shared subscription per MQTT spec 4.8.2
        let topic_filter = if self.topic.starts_with(SHARED_SUBSCRIPTION_PREFIX) {
            self.topic.clone()
        } else {
            let tf = format!(
                "{}/{}/{}",
                SHARED_SUBSCRIPTION_PREFIX, &ctx.task_info.job_id, self.topic
            );
            tracing::debug!("Prefer shared subscription {} to {}", tf, self.topic);
            tf
        };

        match client.subscribe(topic_filter, self.qos).await {
            Ok(_) => (),
            Err(e) => {
                return Err(UserError {
                    name: "MqttSourceError".to_string(),
                ...
                event = eventloop.poll() => {
                    match event {
                        Ok(MqttEvent::Incoming(Incoming::Publish(p))) => {
                            let topic = String::from_utf8_lossy(p.topic.as_ref()).to_string();
